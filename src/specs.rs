...
impl Specs {
    pub fn for_level(level: usize) -> Self {
        ...
        if level == 1 {
            ...
            monsters = 3;
            potions = 50;
        } else if level%10 == 0 { // multiples of 10 are without monster
            width = (29 + level*5/3).min(600);
            height = width * 2 / 3;
            lives = 1;
            potions = 0;
            monsters = 0;
        } else if level%10 == 3 && level > 3 { // no potions, but many cuts
        ...
        } else if level>15 && level%7 == 0 { // easy levels, for a change
            width = 10 + level%13;
            height = width * 2 / 3;
            monsters = 3;
            potions = 5;
